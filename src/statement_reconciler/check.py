...
def check_folder(
    ...
    document_records, unread = read_pdf(files.document, source.document, source.date_order)
    spreadsheet_records = read_records(files.spreadsheet, source.records, source.date_order)
    ...
    if not document_records:
        # Almost always a parsing problem rather than a genuinely empty statement: a scanned or
        # image-only PDF has no text layer to read, and wrong column positions put every word
        # outside every column. Either way nothing was read, so nothing can be confirmed.
        result = replace(
            result,
            warnings=(
                f"No records could be read from {files.document.name} "
                f"({source.label('document')}). If it is a scan or a "
                f"photo it has no text to read; otherwise check document.columns with "
                f"`reconcile probe`.",
            ),
        )
    ...
    if write:
        out_path = write_report(
            result, folder / REPORT_NAME, source.name, folder, labels=source.labels
        )
