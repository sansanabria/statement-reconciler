...
class _SizeIndex:
    """Files already on disk, grouped by size, so re-sends can be spotted without re-walking.

    Senders re-send the same statement, sometimes days later and under a different filename, so
    the check has to be on content rather than name. Doing that naively means walking the whole
    tree once per attachment, which grows with every day filed; the tree is walked once per root
    instead and kept up to date as files are written.
    """
    ...
    def _scan(self, root: Path) -> None:
        if root in self._scanned or not root.is_dir():
            self._scanned.add(root)
            return
        for existing in root.rglob("*"):
            if existing.is_file():
                self._by_size.setdefault(existing.stat().st_size, []).append(existing)
        self._scanned.add(root)

    def add(self, path: Path) -> None:
        self._by_size.setdefault(path.stat().st_size, []).append(path)

    def duplicate_of(self, candidate: Path, folder: Path) -> Path | None:
        """An identical file already filed under the same root, or None."""
        root = folder.parent if folder.parent.exists() else folder
        self._scan(root)
        size = candidate.stat().st_size
        for existing in self._by_size.get(size, ()):
            if existing != candidate and existing.is_file():
...
def fetch(
    ...
    for folder in folders:
        for message in _restrict(folder.Items, start, end):
            ...
            _save_attachments(config, source, message, report, dry_run, index)
...
def _save_attachments(
    ...
    for attachment in message.Attachments:
        ...
        duplicate = index.duplicate_of(destination, target)
