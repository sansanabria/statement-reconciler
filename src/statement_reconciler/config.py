...
from .normalize import DATE_ORDERS, DAY_FIRST
...
#: What the two sides are called in the report when the config does not say. They describe the
#: FORMAT, not who owns the file -- neither side has to be yours. When both sides come from
#: outside, name them in `labels:` so the report says which is which.
DEFAULT_LABELS = {"document": "document", "spreadsheet": "spreadsheet"}
...
class Source:
    ...
    #: Display names for the two sides. Purely presentational -- the reconciliation is identical
    #: either way -- but when neither file is yours, "Broker statement" beats "spreadsheet".
    ...
    #: 03/04/2026 is 3 April or 4 March, so the source declares it.
    date_order: str = DAY_FIRST

    def label(self, side: str) -> str:
        return self.labels.get(side, DEFAULT_LABELS.get(side, side))
    ...
    def from_dict(data: Any, index: int) -> Source:
        ...
        name = name.strip()
        # The name becomes a folder under output_root, so a separator or a parent reference in
        # it would write outside the tree the config asked for.
        if any(part in name for part in ("/", "\\", "..")) or Path(name).is_absolute():
            raise ConfigError(
                f"{where}.name: {name!r} cannot be used as a folder name. It must not contain "
                f"a path separator or '..' -- the name becomes a directory under output_root."
            )
        ...
        labels = _parse_labels(data.get("labels"), f"{where}.labels")
        date_order = data.get("date_order", DAY_FIRST)
        if date_order not in DATE_ORDERS:
            raise ConfigError(
                f"{where}.date_order: expected one of {', '.join(DATE_ORDERS)}, got "
                f"{date_order!r}. It decides whether 03/04/2026 is 3 April or 4 March."
            )
...
def _parse_labels(data: Any, where: str) -> dict[str, str]:
    """Display names for the two sides, defaulting to the format words."""
    labels = dict(DEFAULT_LABELS)
    if data is None:
        return labels
    data = _require_mapping(data, where)
    _reject_unknown(data, tuple(DEFAULT_LABELS), where)
    for side, value in data.items():
        if not isinstance(value, str) or not value.strip():
            raise ConfigError(f"{where}.{side}: expected a non-empty name")
        labels[side] = value.strip()
    return labels
