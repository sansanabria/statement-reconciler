...
class TestLabels:
    """Neither side has to be yours, so the report must be able to name both."""

    def test_defaults_describe_the_format(self, config):
        source = config.sources[0]
        assert source.label("document") == "document"
        assert source.label("spreadsheet") == "spreadsheet"

    def test_labels_can_be_named(self, config_dict):
        config_dict["sources"][0]["labels"] = {
            "document": "Broker statement",
            "spreadsheet": "Insurer commission file",
        }
        source = parse_config(config_dict).sources[0]
        assert source.label("document") == "Broker statement"
        assert source.label("spreadsheet") == "Insurer commission file"

    def test_one_side_can_be_named_alone(self, config_dict):
        config_dict["sources"][0]["labels"] = {"document": "Broker statement"}
        source = parse_config(config_dict).sources[0]
        assert source.label("document") == "Broker statement"
        assert source.label("spreadsheet") == "spreadsheet"

    def test_an_unknown_side_is_rejected(self, config_dict):
        config_dict["sources"][0]["labels"] = {"ledger": "Ours"}
        with pytest.raises(ConfigError, match="unknown key"):
            parse_config(config_dict)

    def test_a_blank_label_is_rejected(self, config_dict):
        config_dict["sources"][0]["labels"] = {"document": "  "}
        with pytest.raises(ConfigError, match="non-empty name"):
            parse_config(config_dict)


class TestDateOrder:
    """Nothing in a document says whether 03/04/2026 is 3 April or 4 March, so the source does."""

    def test_defaults_to_day_first(self, config):
        assert config.sources[0].date_order == "day_first"

    def test_month_first_can_be_set(self, config_dict):
        config_dict["sources"][0]["date_order"] = "month_first"
        assert parse_config(config_dict).sources[0].date_order == "month_first"

    def test_an_unknown_order_is_rejected(self, config_dict):
        config_dict["sources"][0]["date_order"] = "yyyy-first"
        with pytest.raises(ConfigError, match="expected one of"):
            parse_config(config_dict)


class TestSourceNameIsAFolderName:
    """The name becomes a directory under output_root, so it must be safe to use as one."""

    @pytest.mark.parametrize("name", ["../escape", "a/b", "a" + chr(92) + "b", "..", "/abs"])
    def test_path_like_names_are_rejected(self, config_dict, name):
        config_dict["sources"][0]["name"] = name
        with pytest.raises(ConfigError, match="cannot be used as a folder name"):
            parse_config(config_dict)

    def test_ordinary_names_are_fine(self, config_dict):
        config_dict["sources"][0]["name"] = "BANK X (EUR)"
        assert parse_config(config_dict).sources[0].name == "BANK X (EUR)"

    def test_the_name_is_trimmed(self, config_dict):
        config_dict["sources"][0]["name"] = "  ACME  "
        assert parse_config(config_dict).sources[0].name == "ACME"
