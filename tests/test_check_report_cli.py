...
class TestUnreadableDocument:
    """A document nothing could be read from must never come out green.

    A scanned or photographed PDF has no text layer, so it yields no records at all -- and two
    empty sides trivially "agree". Reporting that as reconciled would be a silent pass on a
    document nobody actually read.
    """

    @pytest.fixture
    def scanned_folder(self, run_folder):
        from reportlab.lib.pagesizes import A4
        from reportlab.pdfgen import canvas

        (run_folder / "statement.pdf").unlink()
        page = canvas.Canvas(str(run_folder / "scan.pdf"), pagesize=A4)
        page.rect(50, 600, 400, 150, fill=1)  # graphics only, no text
        page.save()
        return run_folder

    def test_it_warns_instead_of_passing_silently(self, scanned_folder, config):
        result, _ = check_folder(scanned_folder, config)
        assert result.warnings and "No records could be read" in result.warnings[0]

    def test_two_empty_sides_do_not_reconcile(self, scanned_folder, config):
        from openpyxl import Workbook

        (scanned_folder / "records.xlsx").unlink()
        book = Workbook()
        book.active.append(["Date", "Description", "Quantity", "Amount"])
        book.save(scanned_folder / "records.xlsx")

        result, _ = check_folder(scanned_folder, config)
        assert result.document_count == 0 and result.spreadsheet_count == 0
        assert not result.reconciled

    def test_the_warning_reaches_the_summary_sheet(self, scanned_folder, config):
        _, out_path = check_folder(scanned_folder, config)
        sheet = load_workbook(out_path)["Summary"]
        labels = [row[0].value for row in sheet.iter_rows(max_col=1)]
        assert "WARNING" in labels

    def test_a_readable_document_carries_no_warning(self, run_folder, config):
        result, _ = check_folder(run_folder, config)
        assert result.warnings == ()


class TestLabelledSides:
    """When both files come from outside, the report must say which is which."""

    @pytest.fixture
    def labelled_config(self, config_dict, tmp_path):
        from statement_reconciler.config import parse_config

        config_dict["output_root"] = str(tmp_path)
        config_dict["sources"][0]["labels"] = {
            "document": "Broker statement",
            "spreadsheet": "Insurer file",
        }
        return parse_config(config_dict)

    def test_summary_counts_use_the_labels(self, run_folder, labelled_config):
        _, out_path = check_folder(run_folder, labelled_config)
        sheet = load_workbook(out_path)["Summary"]
        labels = [row[0].value for row in sheet.iter_rows(max_col=1)]
        assert "Records in Broker statement" in labels
        assert "Records in Insurer file" in labels

    def test_unmatched_names_the_side_that_has_it(self, make_run_folder, labelled_config):
        folder = make_run_folder(ROWS, ROWS[:2])
        _, out_path = check_folder(folder, labelled_config)
        body = list(load_workbook(out_path)["Unmatched"].iter_rows(min_row=2, values_only=True))
        assert body[0][0] == "Broker statement" and body[0][1] == "Insurer file"

    def test_the_reconciliation_itself_is_unaffected(self, run_folder, labelled_config, config):
        """Labels are presentation only -- the verdict must not depend on them."""
        plain, _ = check_folder(run_folder, config, write=False)
        labelled, _ = check_folder(run_folder, labelled_config, write=False)
        assert plain.reconciled == labelled.reconciled
        assert len(plain.matched) == len(labelled.matched)
